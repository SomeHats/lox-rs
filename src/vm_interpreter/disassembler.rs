...
pub fn disassemble(fun: &FunctionObj) {
    println!("== {} ==", fun.name());
    let mut offset = 0;
    while offset < fun.chunk().code().len() {
        match disassemble_instruction_at(fun, offset) {
...
pub fn disassemble_instruction_at(
    ...
    let (mut offset, op_code) = fun.chunk().read_op_code(initial_offset)?;
    let source_info = fun
        .chunk()
        .read_op_debug(initial_offset)
        .map(|op_debug| {
            Colorize::clear(get_formatted_line(fun.chunk().source().str(), op_debug).as_str())
        })
        .unwrap();
    ...
    match op_code {
        ...
        | OpCode::GreaterThanOrEqualTo => {
            print_line([
                prefix_data(fun, initial_offset, offset),
                (Some(" | ".into()), 3),
        ...
        OpCode::Jump | OpCode::JumpIfTrue | OpCode::JumpIfFalse => {
            let (next_offset, arg) = fun.chunk().read_u16(offset)?;
            offset = next_offset;
        ...
        OpCode::ReadLocal | OpCode::SetLocal => {
            let (next_offset, local_index) = fun.chunk().read_u8(offset)?;
            offset = next_offset;
            print_line([
                prefix_data(fun, initial_offset, offset),
                (Some(" | ".into()), 3),
        ...
        OpCode::Constant | OpCode::DefineGlobal | OpCode::ReadGlobal | OpCode::SetGlobal => {
            let (next_offset, address) = fun.chunk().read_constant_address(offset)?;
            offset = next_offset;
            let value = fun.chunk().get_constant_value(address)?;
...
fn prefix_data(
    ...
    let string = format!(
        ...
        (initial_offset..current_offset)
            .into_iter()
            .map(|offset| format!("{:02x}", fun.chunk().read_u8(offset).unwrap().1))
            .join(" ")
    );
