...
lazy_static! {
    ...
    static ref BYTECODE_IGNORE_PATTERN: Regex = Regex::new("test_fixtures/((call|class|closure|constructor|field|function|inheritance|limit|method|regression|return|super|this)|(comments/(line_at_eof|unicode)|precedence|redefine_panic|operator/(equals_class|equals_method|not_class|not)|number/(nan_equality)|assignment/to_this|block/empty|variable/(early_bound|local_from_method|duplicate_parameter|unreached_undefined|collide_with_parameter)|for/(return.*|closure.*|syntax)|while/(return.*|closure.*)).lox$)").unwrap();
}
